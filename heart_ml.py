...
@st.cache_resource
def train_model(df: pd.DataFrame, max_depth, n_estimators):
    """
...
st.sidebar.header("Filtri random forest")

max_depth = st.sidebar.slider(
    label = "Profondità massimo",
    min_value = 3,
    max_value = 10,
    value = 3,
    step = 1
...
# EDA di una Variabile 
with st.expander("PROVA DI ESPANSIONE"):
    
    feature_scleta = st.selectbox(
        label = "Scegli",
    ...
    fig_dist = px.histogram(df, x = feature_scleta)
    col_1.plotly_chart(fig_dist)


    fig_vol = px.violin(df, y = feature_scleta, x = "target")
    col_2.plotly_chart(fig_vol)


# * RF PERFORMANCE ---------------------------------------------------

model, metrics, feature_importances = train_model(df, max_depth, n_estimators)
